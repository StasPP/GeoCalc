...
type
  ...
  TLocPoint = record
    Name       :String;
    N          :Integer;
    X, Y, Z    :array [1..3] of Double;  /// 1 - ИГД/СК 2 - Локал. 3 - ИГД+Проекция
  end;
...
var CoordType :integer;
  ...
  begin
    ...
    BEGIN
      ...
      Log.Add('Выполняю проецирование из '+ Label6.Caption +' в '+ Label7.Caption);
  ...
  begin
    ...
    Log.Add('Комбинаций: '+IntTostr(CombCount));
  ...
  begin
     ...
     /// Погрешности ПО РАЗНОСТЯМ КАЖДОЙ ТОЧКИ   и  общая СКП
  ...
  begin
     ...
     // изолированный анализ каждой точки
...
begin
  ...
  InitialCS;
  MainGrid.Options := MainGrid.Options - [goEditing];
  Log.Add('Начинаю анализ '+TimeToStr(Now));
