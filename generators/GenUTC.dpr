...
begin
  ...
  begin
    ...
    S1.Add('Имя:');
    S1.Add('GSK2011-'+inttostr(i));
    S1.Add('');
    S1.Add('Название:');
    S1.Add('ГСК-2011 зона '+inttostr(i)+' (' + IntToStr( ((i-1)*6) ) + #176+' ВД - '+ IntToStr(i*6)+#176+' ВД)');
    S1.Add('');
    S1.Add('Датум:');
    S1.Add('GSK2011');
    S1.Add('');
    ...
    S1.Add('');
    S1.Add('Категория:');
    S1.Add('ГСК-2011 Проекция Гаусса-Крюгера');
    S1.Add('');
    S1.Add('Начальный меридиан:');
    S1.Add(inttostr( trunc((i-0.5)*6)) );
    S1.Add('');
    S1.Add('Начальная широта:');
    S1.Add('0');
    S1.Add('');
    S1.Add('Масштаб:');
    S1.Add('1');
    S1.Add('');
    S1.Add('Условное смещение на Восток:');
    S1.Add(inttostr(trunc((i+0.5)*1000000)));
    S1.Add('');
    S1.Add('Условное смещение на Север:');
    S1.Add('0');
    S1.Add('');
    S1.SaveToFile('GSK11-'+substr+inttostr(i)+'.txt');
  end;
 ...
 { for i := 4 to 30 do
  begin
    ...
    S1.Add('');
    S1.Add('Название:');
    S1.Add('СК-42 зона '+inttostr(i)+' (' + IntToStr( ((i-1)*6) ) + #176+' ВД - '+ IntToStr(i*6)+#176+' ВД)');
    S1.Add('');
    S1.Add('Датум:');
    S1.Add('SK42');
    S1.Add('');
    ...
    S1.Add('');
    S1.Add('Категория:');
    S1.Add('СК42 Проекция Гаусса-Крюгера');
    S1.Add('');
    S1.Add('Начальный меридиан:');
    S1.Add(inttostr( trunc((i-0.5)*6)) );
    S1.Add('');
  ...
  begin
    ...
    S1.Add('');
    S1.Add('Название:');
    S1.Add('СК-95 зона '+inttostr(i)+' (' + IntToStr( ((i-1)*6) ) + #176+' ВД - '+ IntToStr(i*6)+#176+' ВД)');
    S1.Add('');
    S1.Add('Датум:');
    S1.Add('SK95');
    S1.Add('');
    ...
    S1.Add('');
    S1.Add('Категория:');
    S1.Add('СК95 Проекция Гаусса-Крюгера');
    S1.Add('');
    S1.Add('Начальный меридиан:');
    S1.Add(inttostr( trunc((i-0.5)*6)) );
    S1.Add('');
  ...
  begin
    ...
    if i<30 then substr2:='ЗД'
      else substr2:= 'ВД';
    j := (i-1)*6 ;
      if j>=180 then
        j:= j-180;
                            
    S1.Add('UTM (WGS-84) зона '+inttostr(i)+'N (' + IntToStr(j) +#176+ ' '
          +substr2+' - '+ IntToStr(j+6)+#176+' '+substr2+'), Северное полушарие');
    S1.Add('');
    S1.Add('Датум:');
    S1.Add('WGS84');
    S1.Add('');
  ...
  repeat
    ...
    S1.Add('');
    S1.Add('Категория:');
    S1.Add('Местные системы координат субъектов РФ');
    S1.Add('');
    shift := 0;
    if GetCols(s2[i],',',2,1)='9999'
      then
